...
date: 2023-08-26T15:00:00-04:00
...
## `auth_request` (Organizr authentication API) method
...
### Problem 1: The Lua scripts

HAProxy doesn't have `auth_request` functionality by default, but there *is* a good Lua plugin that will handle it. Unfortunately, this is also the first time the OPNsense GUI gets in our way.
...
Then you can just add the output file to HAProxy under the "Advanced" tab in OPNsense. One problem solved!

### Problem 2: The auth request
...
In a normal, manually written HAProxy config, this would be pretty simple to do in either the backend or the frontend:
...
See how simple those are? Wouldn't it be nice if things were that simple for us?

Unfortunately, there's no way to do either of these with the OPNsense HAProxy GUI (at least without making the GUI redundant). We'll be relying on option pass-through a couple times, once in the frontend and once in the backend, but the option pass-throughs are tacked on too late in the configuration blocks to use them for a *simple* config: the pass-through in the frontend will be placed at the very end of the block, and the pass-through in the backend will be placed directly after your rules. As the option pass-through is the only way to add in the option to perform the auth request, we'll need to get a little more creative.
...
We'll actually be using the built-in condition and rule management for this, which is what makes this setup the best option here. First off, create a new condition to match on failed auth:
...
## Javascript Web Token (JWT) method

Just like the other method, because we have to use option pass-through, we'll need to divide the settings here between the frontend and the backend. Thankfully, there's much less work to perform in the frontend here, and you could *probably* do this all in the backend by accessing things directly rather than extracting fields like I am here; I based this on the four sample lines from the HAProxy documentation for [`jwt_verify`](https://cbonte.github.io/haproxy-dconv/2.5/configuration.html#7.3.1-jwt_verify), and I didn't try to optimize it.
